...
Pravidla s `$match-case` jsou podporována aplikací AdGuard pro iOS a Safari s **SafariConverterLib v2.0.43 nebo novějším**.
...
V AdGuardu pro iOS a Safari fungují pravidla `$urlblock` jako [$document exclusion](#document-modifier) — odblokování všeho.
...
V AdGuardu pro iOS a Safari fungují pravidla `$genericblock` jako [$document exclusion](#document-modifier) — odblokování všeho.
...
Pravidla s modifikátorem `$specifichide` nejsou AdGuardem pro iOS, Safari a Blokátorem obsahu AdGuard podporována.
...
Syntaxe `$removeparam` je podporována počínaje verzí [CoreLibs](https://adguard.com/en/blog/introducing-corelibs.html) v1.7 a Rozšířením prohlížeče AdGuard v3.6.
...
1. `Aplikace CoreLibs` — AdGuard pro Windows, macOS, Android
...
1. `AdGuard pro Safari` — AdGuard pro desktopový prohlížeč Safari
1. `Blokátor obsahu AdGuard` — blokátor obsahu pro mobilní prohlížeče Android: Samsung Internet a Yandex Browser
